...
impl ReconnectController {
    ...
    pub async fn reconnect_saved_servers(&self) -> Vec<ReconnectResult> {
        ...
        let servers = match self.saved_servers.read() {
            Ok(s) => s.clone(),
        ...
        let ipc_override = match self.ipc_socket_path_override.lock() {
            Ok(g) => g.clone(),
            Err(e) => e.into_inner().clone(),
        };
        ...
        // Build plans for remembered non-local servers
        ...
        // Drop the credential provider lock before executing plans
    ...
    pub async fn reconnect_server(&self, server_id: String) -> ReconnectResult {
        ...
        if let Some(server) = saved_server {
            ...
            if let Some(plan) = compute_reconnect_plan(&server, credential.as_ref(), false) {
                ...
                return execute_reconnect_plan(&plan, &self.inner, ipc_override).await;
            }
        }

        // Fallback: if server exists in snapshot but not in saved list,
        // try a direct remote connection using snapshot data
