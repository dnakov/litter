...
internal class JsonRpcWebSocketClient(
    ...
    fun request(
        ...
        try {
            sendFrame(opcode = 0x1, payload = payload.toString().toByteArray(StandardCharsets.UTF_8))
        } catch (error: Throwable) {
            pending.remove(id)
    ...
    fun addNotificationListener(listener: (String, JSONObject) -> Unit): Int {
        val id = listenerCounter.getAndIncrement()
    ...
    private fun readFrame(input: InputStream): Frame {
        ...
        if (mask != null) {
            for (idx in payload.indices) {
                payload[idx] = (payload[idx].toInt() xor mask[idx % 4].toInt()).toByte()
            }
        }
    ...
    private fun handleMessage(text: String) {
        ...
        if (envelope.has("method") && !envelope.has("result") && !envelope.has("error")) {
    ...
    private fun handleNotification(envelope: JSONObject) {
        val method = envelope.optString("method").trim()
        if (method.isEmpty()) {
            return
        }
        val params =
            when (val value = envelope.opt("params")) {
                null, JSONObject.NULL -> JSONObject()
                is JSONObject -> value
                else -> JSONObject().put("value", value)
            }
        for (listener in notificationListeners.values) {
            runCatching { listener(method, params) }
        }
    }
    ...
    private fun handleServerRequest(envelope: JSONObject) {
        ...
        runCatching {
            sendFrame(opcode = 0x1, payload = response.toString().toByteArray(StandardCharsets.UTF_8))
        }
    }
